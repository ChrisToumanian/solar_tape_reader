      subroutine cmpdiff(imgred, imgblu, imgdiff, ix, jx
     &,                 redellips, bluellips, xc, yc, fdiff)
c
c     compute a diff from red & blu images
c
      integer*2 imgred(ix,jx), imgblu(ix,jx), imgdiff(ix,jx)
      dimension redellips(5), bluellips(5)
     &,         uxr(4), uyr(4)
     &,         uxb(4), uyb(4)
     &,         xmgred(1024), xmgblu(1024)
c
      if(ix.gt.1024) stop '-STOP- : cmpdiff buffer too small'
c
      dxr = redellips(3)-xc
      idr = dxr
      dxr = dxr - idr
...
c 
      ...
      je = min(je,jx-jdb-2)
c
c     now translate and compute the diff
c
      do 100 j = js, je
        jr0 = j + jdrm2
        jb0 = j + jdbm2
...
c       diff = diff*fdiff + 0.5
        diff = max(-1.0,min(+1.0,dif))
   80   imgdiff(i,j) = diff*fdiff
