      subroutine cmpsum(imgred, imgblu, imgsum, ix, jx
     &,                 redellips, bluellips, xc, yc, fsum)
c
c     compute a sum from red & blu images
c
      integer*2 imgred(ix,jx), imgblu(ix,jx), imgsum(ix,jx)
      dimension redellips(5), bluellips(5)
     &,         uxr(4), uyr(4)
     &,         uxb(4), uyb(4)
     &,         xmgred(1024), xmgblu(1024)
c
      if(ix.gt.1024) stop '-STOP- : cmpsum buffer too small'
c
      dxr = redellips(3)-xc
      idr = dxr
      dxr = dxr - idr
...
c 
      ...
      je = min(je,jx-jdb-2)
c
c     now translate and compute the sum
c
      do 100 j = js, je
        jr0 = j + jdrm2
        jb0 = j + jdbm2
   ...
   50   continue
        ...
        sum = sum*fsum
   80   imgsum(i,j) = min(32760.0,max(-32760.0,sum))
